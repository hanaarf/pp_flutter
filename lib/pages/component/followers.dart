...
// Halaman Pengikut
class PengikutPage extends StatefulWidget {
  final int userId;
  const PengikutPage({Key? key, required this.userId}) : super(key: key);

  @override
  State<PengikutPage> createState() => _PengikutPageState();
}

class _PengikutPageState extends State<PengikutPage> {
  ...
  Future<void> fetchFollowers() async {
    try {
      final data = await _repo.getFollowers(widget.userId);
      if (!mounted) return;
      setState(() {
        following = data;
        isLoading = false;
